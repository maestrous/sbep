...
function ENT:Initialize()
	...
	self.Entity:SetModel( "models/Spacebuild/Nova/machuge.mdl" ) 
	self.Entity:SetName("Big MAC")
...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "SF-BigMAC" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	ent.SPL = ply
...
function ENT:Think()
	...
	if self.Charge > 0 then
		if CurTime() >= self.SpTime then
			...
			Sparky:SetPos(self.Entity:GetPos() + (self.Entity:GetForward() * math.Rand(-1347,120)) + (self.Entity:GetUp() * math.Rand(-125,100)) + (self.Entity:GetRight() * math.Rand(-90,90)) )
			Sparky:Spawn()
	...
	if CurTime() <= self.CDown then
		if CurTime() >= self.SmTime then
			...
			local effectdata = EffectData()
			effectdata:SetOrigin(self.Entity:GetPos() +  self.Entity:GetForward() * math.Rand(-1347,120))
			effectdata:SetStart(self.Entity:GetPos() +  self.Entity:GetForward() * math.Rand(-1347,120))
