...
function ENT:Think()
	...
	if self.PassPod and self.PassPod:IsValid() then
		self.CPL2 = self.PassPod:GetPassenger()
		if (self.CPL2 && self.CPL2:IsValid()) then
	...
	local C = 0 
	if (self.CPL && self.CPL:IsValid()) then C = C + 1 end
	if (self.CPL2 && self.CPL2:IsValid()) then C = C + 1 end
