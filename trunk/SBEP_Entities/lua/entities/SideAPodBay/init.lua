...
function ENT:Think()
	...
	if (self.NPod3 == nil || !self.NPod3:IsValid() || self.NPod3.Active) then
		local ent = ents.Create( "BoardingPod" )
		ent:SetPos( Vector( 100000,100000,100000 ) )
		ent:Spawn()
		ent:Initialize()
	...
	if (self.NPod4 == nil || !self.NPod4:IsValid() || self.NPod4.Active) then
		local ent = ents.Create( "BoardingPod" )
		ent:SetPos( Vector( 100000,100000,100000 ) )
		ent:Spawn()
		ent:Initialize()
...
function ENT:BuildDupeInfo()
	...
	if (self.NPod3) and (self.NPod3:IsValid()) then
	    info.NPod3 = self.NPod3:EntIndex()
	end
	if (self.NPod4) and (self.NPod4:IsValid()) then
	    info.NPod4 = self.NPod4:EntIndex()
	end
	return info
end
...
function ENT:ApplyDupeInfo(ply, ent, info, GetEntByID)
	...
	GetEntByID(info.NPod3):Remove()
	GetEntByID(info.NPod4):Remove()
end
