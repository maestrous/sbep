...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "SF-SmugglerHold3" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	ent.SPL = ply
...
duplicator.RegisterEntityModifier( "SBEPSmuggler3" , function() end)

function ENT:PostEntityPaste(pl, Ent, CreatedEntities)
	local DI = Ent.EntityMods.SBEPSmuggler3
	...
	if(Ent.EntityMods and Ent.EntityMods.SBEPSmuggler3.WireData) then
		WireLib.ApplyDupeInfo( pl, Ent, Ent.EntityMods.SBEPSmuggler3.WireData, function(id) return CreatedEntities[id] end)
	end
