...
function ENT:Think()
	...
	if self.Perch && self.Perch:IsValid() then
		if self.Perch.resources2 then --RD_CheckResource( self.Perch, "energy" )
			local e = RD_GetResourceAmount( self.Perch, "energy" )
			if e >= Delta * 10 then
				RD_ConsumeResource( self.Perch, "energy", Delta * 10 * self.EnergyGain)
				self:AddEn(Delta * 10 * self.EnergyGain)
			else
				RD_ConsumeResource( self.Perch, "energy", e )
				self:AddEn(e)
			end
			local a = RD_GetResourceAmount( self.Perch, "air" )
			if a >= Delta * 10 then
				RD_ConsumeResource( self.Perch, "air", Delta * 10 * self.EnergyGain)
				self:AddEn(Delta * 10 * self.EnergyGain)
			else
				RD_ConsumeResource( self.Perch, "air", a )
				self:AddEn(a)
			end
			local a = RD_GetResourceAmount( self.Perch, "coolant" )
			if a >= Delta * 10 then
				RD_ConsumeResource( self.Perch, "coolant", Delta * 10 * self.EnergyGain)
				self:AddEn(Delta * 10 * self.EnergyGain)
			else
				RD_ConsumeResource( self.Perch, "coolant", a )
				self:AddEn(a)
			end	
		end
	end
