...
function ENT:Draw()
	...
	if self.Blip1 > 0 then
		...
		render.DrawSprite( self.Entity:GetPos() + self.Entity:GetRight() * 150 + self.Entity:GetForward() * 30, self.Blip1, self.Blip1, color )
		--self.Blip1 = self.Blip1 - 50
	end
	if self.Blip2 > 0 then
		local color = Color( 60, 60, 200, self.Blip2 )
		render.DrawSprite( self.Entity:GetPos() + self.Entity:GetRight() * -150 + self.Entity:GetForward() * 30, self.Blip2, self.Blip2, color )
	...
	if self:GetActive() then
		...
		if self.Blip1 == 255 then
			...
			if ( dlight ) then
				...
				dlight.Pos = self:GetPos() + self:GetRight() * 150
				dlight.r = 200
				dlight.g = 60
				dlight.b = 60
				dlight.Brightness = 7
