...
REM /models/ directory when executed from any model subfolder.
ECHO Finding root model directory...
:FindModelRoot
REM Infinite loop avoidance if it can't find a folder named "models" in less than 25 steps.
...
REM Main loop to find all .mdl files in specified scope and write them to the named file with a number sequence.
...
REM Loop assistant, file writing and string substitution subroutine 
:WriteStep
