...
function ENT:Initialize()
	...
	util.SpriteTrail( self.Entity, 0,  Color(50,50,200,200), false, 50, 0, 3, 1, "trails/smoke.vmt" )
...
function ENT:Think()
	...
	if (self.DTimer == true) then
		if (CurTime() >= self.DTime) then
			...
			ShakeIt:SetKeyValue("radius", "100" )
		...
		if (data.HitEntity:IsWorld() || data.HitEntity:IsPlayer() || data.HitEntity:IsNPC()) then
			...
			if (data.HitEntity != self.LastHit) then
				...
				if (attack == 0) then
					...
					local ShakeIt = ents.Create( "env_shake" )
					ShakeIt:SetName("Shaker")
					ShakeIt:SetKeyValue("amplitude", "50" )
					ShakeIt:SetKeyValue("radius", "60" )
