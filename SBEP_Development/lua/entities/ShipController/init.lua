...
function ENT:Initialize()
	...
	local inNames = {"MoveVector", "TargetVector", "Angle", "Stance", "TargetFound", "WaypointRadius"}
	local inTypes = {"VECTOR","VECTOR","ANGLE","NORMAL","NORMAL"}
	self.Inputs = WireLib.CreateSpecialInputs( self.Entity,inNames,inTypes)
	self.Outputs = Wire_CreateOutputs( self.Entity, { "Pitch", "Yaw", "Roll", "Forward", "WaypointReached" })
...
function ENT:Think()
	...
	if self.Stance > 0 then --Dear god, this is a mess... Must remember to organize this function better.
		if (self.Stance < 3) || (self.Stance == 3 && !self.TFound) then
			if self.MVec != Vector(0,0,0) then
				...
				if MDist < 500 then
					...
					self.Forward = math.Clamp(MDist * 0.05, 0, 500)
				end
			end
		elseif self.Stance == 3 && self.TFound then
			...
			if MDist > 1000 then
				...
				self.Forward = math.Clamp(MDist * 0.1, 0, 500)
			else
				local HighP = 0
				local MainG = 0
